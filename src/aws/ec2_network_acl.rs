...
use rusoto_ec2::{DeleteNetworkAclRequest, Ec2, Ec2Client};
use tracing::{debug, trace};

#[derive(Clone)]
pub struct Ec2NetworkAclClient {
    client: Ec2Client,
    region: Region,
    account_num: String,
...
#[async_trait]
impl NukerClient for Ec2NetworkAclClient {
    async fn scan(&self) -> Result<Vec<Resource>> {
        trace!("Initialized EC2 Network ACL resource scanner");
    ...
    async fn delete(&self, resource: &Resource) -> Result<()> {
        self.delete_nacl(resource).await
    }
}
