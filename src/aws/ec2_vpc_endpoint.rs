...
use rusoto_ec2::{DeleteVpcEndpointsRequest, Ec2, Ec2Client};
use tracing::{debug, trace};

#[derive(Clone)]
pub struct Ec2VpcEndpointClient {
    client: Ec2Client,
    region: Region,
    account_num: String,
...
#[async_trait]
impl NukerClient for Ec2VpcEndpointClient {
    async fn scan(&self) -> Result<Vec<Resource>> {
        trace!("Initialized EC2 VPC Endpoint resource scanner");
    ...
    async fn delete(&self, resource: &Resource) -> Result<()> {
        self.delete_endpoint(resource).await
    }
}
